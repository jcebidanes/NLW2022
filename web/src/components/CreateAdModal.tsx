import * as Dialog from '@radix-ui/react-dialog' //importando tudo como um objeto
import { Input } from './Form/Input'
import * as Checkbox from '@radix-ui/react-checkbox' //importando tudo como um objeto
import {Check, GameController, MagnifyingGlassPlus} from 'phosphor-react'
import * as ToggleGroup from '@radix-ui/react-toggle-group' //importando tudo como um objeto
              ...
              <form onSubmit={handleCreateAd} className='mt-8 flex flex-col gap-4'>
                <div className='flex flex-col gap-2'>
                  ...
                  <select defaultValue="" name="game" id="game" className='bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 appearance-none' >
                    <option disabled  value="">Selecione o game que dseja jogar</option>
                    { games.map(game => {
                        return(
                ...
                <div className='flex flex-col gap-2'>
                  <label htmlFor="game">Seu nome/nickname</label>
                ...
                <label  className='mt-4 flex gap-2 text-sm items-center'>
                    <Checkbox.Root checked={useVoiceChanel} className='w-6 h-6 p-1 rounded bg-zinc-900' onCheckedChange={(checked) => {
                            if (checked === true){
                        ...
                        <Checkbox.Indicator>
                            <Check  className='w-4 h-4 text-emerald-400' id='a' />
                        </Checkbox.Indicator>
                    </Checkbox.Root>
                    Costume me conectar ao aúdio
                </label>

                <footer className='mt-4 flex justify-end gap-4'>
                  <Dialog.Close className='bg-zinc-500 px-5 h-12 rounded-md font-semibold hover:bg-violet-600'>Cancelar</Dialog.Close>
                  <button  className='bg-violet-500 px-5 h-12 rounded-md font-semibold flex items-center gap-3 hover:bg-zinc-600' ><GameController size={24} /> Encontrar duo </button>
                </footer>
